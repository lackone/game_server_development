...
if mode == "slave" then
    ...
    skynet.start(function()
        ...
        for req, resp in skynet.request { slave, "lua", "ping", 6, "SLEEP 6" } { slave, "lua", "ping", 5, "SLEEP 5" } { slave, "lua", "ping", 4, "SLEEP 4" } { slave, "lua", "ping", 3, "SLEEP 3" } { slave, "lua", "ping", 2, "SLEEP 2" } { slave, "lua", "ping", 1, "SLEEP 1" }
                :select() do
            info("RESP %s", resp[1])
        end
        ...
        for req, resp in skynet.request { slave, "lua", "error" } { slave, "lua", "ping", 0, "PING" }
                :select() do
            if resp then
                info("Ping : %s", resp[1])
            else
        ...
        -- call in select
        for req, resp in skynet.request { slave, "lua", "ping", 20, "CALL 20" } { slave, "lua", "ping", 10, "CALL 10" }
                :select() do
