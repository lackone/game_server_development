...
checkerr("invalid position", m.match, function(s, i)
    return i - 1
end, s)
checkerr("invalid position", m.match,
        m.P(1) ^ 0 * function(_, i)
            return i - 1
        end, s)
assert(m.match(m.P(1) ^ 0 * function(_, i)
    return i
end * -1, s))
checkerr("invalid position", m.match,
        m.P(1) ^ 0 * function(_, i)
...
assert(m.match(m.P(function(s, i)
    return s:len() + 1
end) * -1, s))
checkerr("invalid position", m.match, m.P(function(s, i)
    return s:len() + 2
end) * -1, s)
assert(not m.match(m.P(function(s, i)
    return s:len()
end) * -1, s))
assert(m.match(m.P(1) ^ 0 * function(_, i)
    return true
...
p = (m.P(function(s, i)
    return i % 2 == 0 and i
        ...
        + m.P(function(s, i)
    return i % 2 ~= 0 and i + 2 <= s:len() and i
end) * 3) ^ 0
        * -1
...
t = { m.match((m.C(1) / function(x)
    return x, x .. "x"
end) ^ 0, "abc") }
checkeq(t, { "a", "ax", "b", "bx", "c", "cx" })

t = m.match(m.Ct((m.C(1) / function(x, y)
    return y, x
end * m.Cc(1)) ^ 0), "abc")
...
local function isnullable (p)
    checkerr("may accept empty string", function(p)
        return p ^ 0
    end, m.P(p))
end
...
do
    ...
    p = m.Cg(m.Cc(20), 'k') * m.Cg(m.Cc(10) * m.Cb('k') * m.C(1), 'k')
            * (m.Cb('k') / function(a, b, c)
        return a * 10 + b + tonumber(c)
    end)
...
p = m.P 'a' * (function(s, i)
    return (s:sub(i, i) == 'b') and i + 1
end)
        + 'acd'
...
do
    -- run-time capture in an end predicate (should discard its value)
    local x = 0
    function foo (s, i)
        x = x + 1
        return true, x
...
ref = function(s, i, x)
    return i == tonumber(x) and i, 'xuxu'
end
...
c = re.compile([[
  ...
  number <- %d+ -> tonumber
]], { tonumber = tonumber, add = function(a, b)
    return a + b
end })
